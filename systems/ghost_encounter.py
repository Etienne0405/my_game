...
def ghost_encounter():
    ...
    music.play_music("music/battle_music.wav")

    ghost_hp = 5
    pygame.mixer.init()

    low_health_path = os.path.join("music", "health", "health_low.mp3")
    low_health_sound = pygame.mixer.Sound(low_health_path)
    low_health_playing = False

    pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "ghost_entry.wav")).play()
    time.sleep(0.5)
    ...
    while ghost_hp > 0:
        ...
        if attack_type == "scare":
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "scare.mp3")).play()
            typewriter("The ghost tries to scare you with haunting sounds.\n")
        elif attack_type == "attack":
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "fast_attack.wav")).play()
            typewriter("The ghost raises its arm for a quick attack!\n")
        else:
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "charge-attack.wav")).play()
    ...
    pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "ghost_death.mp3")).play()
...
def play_weapon_sound(weapon):
    ...
    if weapon in sounds:
        pygame.mixer.Sound(os.path.join("music", "ghost_encounter", sounds[weapon])).play()
...
def resolve_combat_choice(choice, attack_type, ghost_hp):
    ...
    if choice == "1":
        ...
        if random.random() < hit_chance:
            ...
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "ghost_hurt.mp3")).play()
            typewriter(f"Your {weapon} hits the ghost!\n")
            ghost_hp -= damage
        else:
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "damaged_player.mp3")).play()
        ...
        if random.random() < dodge_chance:
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "dodge.mp3")).play()
            typewriter("You dodge successfully!\n")

            if attack_type == "charge":
                pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "fists.wav")).play()
                pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "ghost_hurt.mp3")).play()
        ...
        if attack_type == "attack":
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "block.mp3")).play()
            pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "ghost_hurt.mp3")).play()
        ...
        pygame.mixer.Sound(os.path.join("music", "ghost_encounter", "potion.mp3")).play()
